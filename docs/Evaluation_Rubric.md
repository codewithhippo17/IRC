...
### Basic Checks — 20 points

| # | Requirement | Notes |
|---|-------------|-------|
| 1 | There is a Makefile, the project compiles correctly with the required options, is written in C++, and the executable is called as expected. | |
| 2 | Ask and check how many `poll()` (or equivalent) are present in the code. There must be **only one**. | |
| 3 | Verify that the `poll()` (or equivalent) is called every time before each `accept`, `read`/`recv`, `write`/`send`. After these calls, `errno` should **not** be used to trigger specific action (e.g., reading again after `errno == EAGAIN`). | |
| 4 | Verify that each call to `fcntl()` is done as follows: `fcntl(fd, F_SETFL, O_NONBLOCK);` Any other use of `fcntl()` is forbidden. | |

> ⚠️ **If any of these points is wrong, the evaluation ends now and the final mark is 0.**
...
- [ ] Stop a client (`Ctrl+Z`) connected on a channel. Then flood the channel using another client. The server should not hang. When the client is live again, all stored commands should be processed normally. Also, check for memory leaks during this operation.

### Client Commands Basic — 20 points
...
### Client Commands Channel Operator — 20 points
...
| Basic Checks | 20 | |
| Networking | 20 | |
| Networking Specials | 20 | |
| Client Commands Basic | 20 | |
| Client Commands Channel Operator | 20 | |
| **Mandatory Total** | **100** | |
| File Transfer (Bonus) | 12.5 | |
| A Small Bot (Bonus) | 12.5 | |
| **Grand Total** | **125** | |
...
- **Total Score : 0** (baseline — to be filled at evaluation)
- Flag for empty repository, non-functioning program, Norm error, cheating, or segfault results in final grade 0 (or -42 for cheating).
- Memory leaks: any heap-allocated memory must be freed before end of execution.
