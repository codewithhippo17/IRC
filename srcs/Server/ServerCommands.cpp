...
void Server::_handleClientData(int fd) {
  ...
  client->appendToBuffer(
      std::string(buf, bytes)); // NOTE: append received data to client's buffer

  std::vector<std::string> messages =
      client->extractMessages(); // NOTE: extract messages from buffer
  ...
  if (sent > 0) {
    ...
    if (!remaining.empty())
      client->appendToSendBuffer(
          remaining); // NOTE: append remaining data to buffer
  }

  if (!client
           ->hasPendingSend()) // NOTE: if there is no data to send, set POLLIN
  {
...
void Server::_processMessage(Client &client, const std::string &message) {
  std::cout << CLR_CYAN << "[<<] fd=" << client.getFd() << ": " << message
            << CLR_RESET << std::endl;
...
void Server::_dispatchCommand(Client &client, const Command &cmd) {
  ...
  if (it != _cmdMap.end()) {
    ...
    client.sendMessage(":" SERVER_NAME " " ERR_UNKNOWNCOMMAND " " +
